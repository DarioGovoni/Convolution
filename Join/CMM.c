/*
 ...
 Description : Implementazione di CMM.h
 ============================================================================
 */
		...
		for(ch=0;ch<image->ch;ch++){
			...
			for(h=0;h<image->h;h++){//scorro le righe
				...
				for(w=0;w<image->w;w++){//scorro colonne
					GET_MATRIX_REAL(result,i,j) = GET_BANK_REAL(image,p,ch,h,w);
		...
		for(ch=0;ch<kernel->ch;ch++){
			...
			for(h=0;h<kernel->h;h++){//scorro le righe
				...
				for(w=0;w<kernel->w;w++){//scorro colonne
					GET_MATRIX_REAL(result,i,j) = GET_BANK_REAL(kernel,p,ch,h,w);
	...
	for(tensore=0;tensore<imageP;tensore++){
		...
		for(canale=0;canale<kernelP;canale++){
			...
			for(h=0;h<imageH;h++){
				int w;
				for(w=0;w<imageW;w++){
					GET_BANK_REAL(result,tensore,canale,h,w) = GET_MATRIX_REAL(out,i,j);
...
matrix * convolveCMM_strassen(matrix *kernel, int kernelSize, matrix *image, int imageSize,int *nsum,int *nmul){
	 ...
	 * matrici; ma ho come PRE che il numero di canali e il parallelismo siano uguali.
	 */
	if(kernel->h == kernelSize && image->h == imageSize){
...
//ATTENZIONE: SELEZIONARE I BLOCCHI E DECOMMENTARE CON UN COLPO SOLO, USANDO CTRL+7
