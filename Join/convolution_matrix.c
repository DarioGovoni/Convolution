/*
 ============================================================================
 Name        : convolution_matrix.c
 Author      : Alessio Onori
 Version     : 1.0
 Copyright   : Your copyright notice
 Description : implementazione di convolution_matrix.h
 ============================================================================
 */
		...
		for(j=j1;j<=j2;j++){//scorro le colonne dell'immagine
			...
			for(x=0;x< kernel->h;x++){//scorro le righe del kernel
				...
				for(y=0;y< kernel->w;y++){//scorro le colonne del kernel
					val+=GET_MATRIX_REAL(immagine,i+m2-x,j+m2-y) * GET_MATRIX_REAL(kernel,x,y);
				}
			}//righe kernel

			GET_MATRIX_REAL(result,i,j)=val;
		}
	}//righe immagine
...
matrix* convolveNaive_border(matrix *kernel, matrix *immagine, int *nsum, int *nmul){
	...
	/*
	 * per non complicare la gestione degli indici risolvo
	 * gestendo gli overflow
	 */
			...
			for(x=0;x< kernel->h;x++){//scorro le righe del kernel
				...
				for(y=0;y< kernel->w;y++){//scorro le colonne del kernel
					...
					if(riga<0 || colonna<0 || riga>i2 || colonna >j2){//overflow significa che sono sul bordo di zeri
						...
						(*nmul)++;
						if( !(x==0 && y==0) )//la primissima somma non si conta
							(*nsum)++;
					}
				}
			}//righe kernel
	...
	for(i=0; i< m1->h; i++){
		...
		for(j=0;j<Hi[i]->w;j++){//colonne di Hi
			for(k=0;k<Hi[i]->h;k++){//righe di Hi
				...
				GET_MATRIX_REAL(Hi[i],riga,j) = GET_MATRIX_REAL(m1,i,k);
			}
			shift++;
		}
	}
...
//	printMatrix(mainH);
	...
	for(i=0;i<m2->h;i++){//righe
		for(j=0;j<m2->w;j++){//colonne
			GET_MATRIX_REAL(m2Colonna,k,0) = GET_MATRIX_REAL(m2,i,j);
			k++;
		}
	}
...
//	printMatrix(m2Colonna);
	 ...
	 */
	matrix *convolution=multiplyMatrix_naive(mainH,m2Colonna,nsum,nmul);//implemento la moltiplicazione come voglio
	...
	for(i=0;i<h;i++){
		for(j=0;j<w;j++){
			GET_MATRIX_REAL(result,i,j)=GET_MATRIX_REAL(convolution,k,0);
			k++;
		}
	}
...
//	printMatrix(m2Colonna);
	 ...
	 * matrici mainH e colonna: le 2 matrici sono compatibili per la moltiplicazione per costruzione
	 */
	matrix *convolution=multiplyMatrix_strassen(mainH,m2Colonna,nsum,nmul);//implemento la moltiplicazione come voglio
	//questo è un vettore colonna
